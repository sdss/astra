...
def _wcs_log_linear(header):
    return (
        10 ** (np.arange(header["NAXIS1"]) * header["CDELT1"] + header["CRVAL1"])
        * u.Angstrom
    )
...
def load_sdss_apstar(path, **kwargs):
    ...
    return Spectrum1D(
        spectral_axis=spectral_axis, flux=flux, uncertainty=uncertainty, meta=meta
    )
        ...
        meta = OrderedDict(
            [
                ...
                (
                    "bitmask",
                    dict(
                        and_mask=image[hdu].data["and_mask"].reshape(flux.shape),
