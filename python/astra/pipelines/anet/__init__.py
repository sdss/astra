...
from astra.pipelines.anet.boss_net import BossNet
...
def anet(
    spectra: Optional[Iterable[ApogeeCoaddedSpectrumInApStar]] = (
        ...
        .select()
        .join(ANet, JOIN.LEFT_OUTER, on=(ApogeeCoaddedSpectrumInApStar.spectrum_pk == ANet.spectrum_pk))
        .where(ANet.spectrum_pk.is_null())
    ),
