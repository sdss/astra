...
def lnlike(theta, data):
    mu, sigma = theta
    return np.sum(-((data - mu) ** 2) / sigma**2 / 2) - len(data) * np.log(sigma)
...
def gauss1d(x, a, b, c):
    return a / (np.sqrt(2.0 * np.pi) * c) * np.exp(-((x - b) ** 2) / c**2 / 2)
...
def test_gfit_bin():
    ...
    theta0 = np.array([1.0, np.median(data), 2 * np.std(data)])
...
def label_diff_bin(label1, label2, plot=False):
    ...
    if plot:
        ...
        for i_dim in range(n_dim):
            ...
            ax.plot(
                bin_edges, gauss1d(bin_edges, amp[i_dim], bias[i_dim], scatter[i_dim])
            )
...
def label_diff_lmfit(label1, label2, bins="auto", bin_std=3, plot=False, emcee=True):
    label1 = np.array(label1)
    label2 = np.array(label2)
    assert label1.shape == label2.shape
    ...
    for i_dim in range(n_dim):
        ...
        theta, frs[i_dim] = gfit_bin_lmfit(data, bins=bins, bin_std=bin_std, plot=False)
