...
from .hyperparameter import (
    summarize_hyperparameters_to_table,
    summarize_table,
...
class Slam3(object):
    ...
    model = "untrained"
    sms = []
    hyperparams = Table(data=[np.zeros(0)] * 3, names=["C", "gamma", "epsilon"])
    scores = np.zeros((0,))
    nmse = None
    stats_train = None
    stats_test = None
    ...
    def __init__(
        ...
        try:
            ...
            self.heal_kwargs = dict(
                mask_conv=mask_conv, flux_bounds=flux_bounds, ivar_eps=ivar_eps
            )
            self.ivar_eps = ivar_eps

        except:
            raise (ValueError("@SLAM: input data error, go back and check your data!"))
        ...
        if scale:
            ...
            self.tr_flux_scaler, self.tr_flux_scaled = standardize(
                tr_flux, tr_ivar, robust=robust
            )
            self.tr_ivar_scaler, self.tr_ivar_scaled = standardize_ivar(
                tr_ivar, self.tr_flux_scaler
            )
            self.tr_labels_scaler, self.tr_labels_scaled = standardize(
                tr_labels, None, robust=robust
            )
    ...
    def hyperparameter_grid_stats(
        self, pivot=("param_C", "param_gamma"), n_jobs=10, verbose=10
    ):
    ...
    def train_pixels(
        ...
        **kwargs:
        ...
        if profile is None:
            ...
            if temp_dir is None:
                print("@Slam: make sure engines and controllers share" " [/tmp]!)")
            else:
                try:
            ...
            dv.execute(
                "(tr_labels_scaled, tr_flux_scaled, sample_weight,"
            ...
            dv.execute(
                "results = train_multi_pixels(tr_labels_scaled,"
            ...
            dv.execute(
                "del(results, tr_labels_scaled, tr_flux_scaled, " "sample_weight)"
            )
    ...
    def predict_labels_quick(
        ...
        if profile is None:
            if tplt_flux is None and tplt_labels is None and tplt_ivar is None:
                ...
                X_quick = predict_labels_chi2(
                    self.tr_flux[::n_sparse, :],
            ...
            dv.execute(
                "tplt_flux, tplt_ivar, tplt_labels, test_flux, "
            ...
            print("@Slam: DO NOT INTERRUPT! Or you have to RESTART ipcluster!")
            dv.execute(
                "X_quick = predict_labels_chi2(tplt_flux, tplt_ivar, "
    ...
    def push_to_ipc(
        ...
        # init ipc
        dv = launch_ipcluster_dv(
            profile=profile, targets=targets, block=block, max_engines=max_engines
        )
        ...
        if count_size:
            print(
                "@Slam: size of me is about {} MB ".format(
    ...
    def predict_labels_ipc(
        ...
        **kwargs
        ...
        # initiate ipcluster
        dv = launch_ipcluster_dv(
            profile=profile, targets=targets, block=True, max_engines=test_flux.shape[0]
        )
        ...
        time2 = time.time()
        print(
            "@Slam: it took so long [{}] ..." "".format(print_time_cost(time2 - time1))
        )
        ...
        ** all input should be 2D array or sequential **
        ...
        if "profile" in kwargs.keys():
            if kwargs["profile"] is None:
                ...
                raise AssertionError("@Slam: use Slam.predict_labels_ipc instead!")
        ...
        test_ivar = np.where(
            np.logical_or(model_ivar < ivar_eps, test_ivar < ivar_eps),
            0.0,
            test_ivar * model_ivar / (test_ivar + model_ivar),
        )
        # fix total_ivar
        test_ivar = np.where(np.isfinite(test_ivar), test_ivar, 0.0)
        ...
        if flux_eps is not None:
            ...
            mask = np.logical_and(mask, test_ivar > 0.0)
        print(
            "@SLAM: The spectra with fewest pixels unmasked is "
        ...
        ** all input should be 2D array or sequential **
        ...
        if labels_scaler is not None:
            ...
            if X_lb is not None:
                ...
                theta_lb = np.ones(X0[0].shape) * -10.0

            if X_ub is not None:
                theta_ub = labels_scaler.transform(X_ub)
            else:
                theta_ub = np.ones(X0[0].shape) * 10.0
        ...
        if profile is None:
            ...
            # initiate ipcluster
            dv = launch_ipcluster_dv(
                profile=profile, targets=targets, block=True, max_engines=n_test
            )
            ...
            dv.execute(
                "X0, svrs, test_flux, test_ivar, mask, kwargs"
                " = load('{}')".format(fp)
            )
            ...
            dv.execute(
                "results_mcmc = [predict_label_mcmc("
        ...
        if labels_scaler is not None:
            for i in range(len(results_mcmc)):
                results_mcmc[i]["theta"] = labels_scaler.inverse_transform(
        ...
        if return_chain and labels_scaler is not None:
            for i in range(len(results_mcmc)):
                results_mcmc[i]["flatchain"] = labels_scaler.inverse_transform(
    ...
    def predict_spectra(
        self, X_pred, labels_scaler=True, flux_scaler=True, n_jobs=1, verbose=False
    ):
        ...
        flux_pred = Parallel(n_jobs=n_jobs, verbose=verbose)(
            delayed(predict_spectrum)(self.sms, X_pred[i]) for i in range(n_pred)
        )
    ...
    def predict_spectra_ppixel(
        self, X_pred, labels_scaler=True, flux_scaler=True, n_jobs=-1, verbose=50
    ):
    ...
    def single_pixel_diagnostic(
        ...
        return single_pixel_diagnostic(
            self.sms,
            i_pixel,
            test_labels,
            diag_dim=diag_dim,
    ...
    def create_mask(self, mask_init, set_range, set_val):
        ...
        for set_range_ in set_range:
            mask_init = np.where(
                np.logical_and(self.wave >= set_range_[0], self.wave <= set_range_[1]),
                set_val,
                mask_init,
            )
    ...
    def correlation_coefs(self):
        ...
        if self.ccoef is None:
            ...
            for i in range(self.n_pix):
                ...
                ccoef[i] = np.abs(
                    np.corrcoef(self.tr_flux[ind_good, i], self.tr_labels[ind_good, 1])[
                        0, 1
                    ]
                )
            self.ccoef = ccoef
    ...
    def plot_training_performance(self, fontsize=20, mse_hline=None):
        ...
        plt.legend(loc=5, framealpha=0.3)
        ...
        if mse_hline is not None:
            plt.hlines(
                mse_hline, self.wave[0], self.wave[-1], linestyle="--", color="gray"
            )
        plt.legend(loc=5, framealpha=0.3)

        fig.sca(axs[2])
        plt.plot(self.wave, np.log10(self.hyperparams["C"]), label="$\\log(C)$")
        plt.plot(
            self.wave, np.log10(self.hyperparams["gamma"]), label="$\\log(\gamma)$"
        )
        plt.plot(
            self.wave, np.log10(self.hyperparams["epsilon"]), label="$\\log(\epsilon)$"
        )
        plt.legend(loc=5, framealpha=0.3)
    ...
    def heal_the_world(
        ...
        if flux_bounds is not None:
            ...
            ind_ivar_reset = np.logical_not(ivar == 0.0) & np.logical_not(
            ...
            print("=====================================================")
            print(
                "@Slam.heal_the_world: IVAR of {} spectra need to be reset"
            ...
            print("=====================================================")
            print(
                "@Slam.heal_the_world: FLUX of {} spectra need to be reset"
            ...
            flux = np.where(ind_flux_reset, 0.0, flux)
            ivar = np.where(ind_ivar_reset, 0.0, ivar)

        if mask_conv is not None:
            for i in range(ivar.shape[0]):
                ivar[i] *= convolve_mask(
                    ivar[i] > 0,
                    kernel_size_coef=0.25,
                    kernel_size_limit=mask_conv,
                    sink_region=None,
                )

        # if labels are bad
        if labels is not None:
            ind_bad_labels = np.any(np.logical_not(np.isfinite(labels)), axis=1)
            n_bad_labels = np.sum(ind_bad_labels)
            if n_bad_labels > 0:
                print("=====================================================")
                print("@Slam.heal_the_world: {} labels are bad".format(n_bad_labels))
                print("=====================================================")
    ...
    def sub_slam(self, ind_sub):
        ...
        return Slam3(
            self.wave,
            self.tr_flux[ind_sub],
            self.tr_ivar[ind_sub],
            self.tr_labels[ind_sub],
            **self.init_kwargs
        )
    ...
    def train_test_split(self, test_size=0.1, train_size=0.9, random_state=0):
        ...
        return Slam3(self.wave, tr_flux, tr_ivar, tr_label, **self.init_kwargs), (
            test_flux,
            test_ivar,
            test_labels,
        )
    ...
    def update(self):
        ...
        s = Slam3(
            self.wave, self.tr_flux, self.tr_ivar, self.tr_labels, **self.init_kwargs
        )
