...
class Lndi(object):
    ...
    def predict_label_rand(
        self, flux_goal, X_init, X_std, n_rand, frac_ext=4, n_0_th=2, verbose=False
    ):
        n_loop = 0
        n_0 = 0
        while True:
            ...
            X_rand_2 = self.generate_label_rand(X_init, X_std * 3.0, n_rand_2)
            ...
            # find chi2_min
            i_min, chi2_min, flux_cont_min = best_chi2(self.wave, flux_rand, flux_goal)
...
def best_chi2(wave, flux, flux_goal, ivar=None):
    flux_cont = determine_continuum(
        wave, flux, flux_goal, norm_range=(4000, 8000), dwave=100, p=(1e-7, 1e-7)
    )
    if ivar is None:
        chi2 = np.nansum((flux * flux_cont - flux_goal) ** 2.0, axis=1)
    else:
        chi2 = np.nansum(
            (flux * flux_cont - flux_goal) ** 2.0 * ivar.reshape(1, -1), axis=1
        )
...
def determine_continuum(
    ...
    flux_cont = normalize_spectra_block(
        wave,
        flux_goal / flux,
