...
class TestTaskBehaviour(unittest.TestCase):
    ...
    def test_task_bundling(self):
        ...
        t = DummyTask(
            A=(1, 2), B="hello", C=(3, 3)
        )  # ok, all non-bundled are same length
        self.assertEqual(t.bundle_size, 2)
        t = DummyTask(
            A=(1, 2, 3), B="hello"
        )  # ok, 3 tasks, all non-bundled are same length, C gets default
    ...
    def test_task_timing(self):
        ...
        class TestTask(TaskInstance):
            ...
            def execute(self):
                for task, input_data_products, parameters in self.iterable():
                    print(
                        f"In {self} with {task} and sleep={parameters['sleep_length']}"
                    )
        ...
        for task, sl in zip(tasks, sleep_length):
            self.assertAlmostEqual(
                task.time_pre_execute_bundle_overhead,
                pre_execute_sleep_length,
                places=places,
            )
            self.assertAlmostEqual(
                task.time_pre_execute, pre_execute_sleep_length, places=places
            )
            ...
            self.assertAlmostEqual(
                task.time_post_execute_bundle_overhead,
                post_execute_sleep_length,
                places=places,
            )
            self.assertAlmostEqual(
                task.time_post_execute, post_execute_sleep_length, places=places
            )
            self.assertAlmostEqual(task.time_post_execute_task, 0, places=places)

            self.assertEqual(
                task.time_total,
                task.time_pre_execute + task.time_execute + task.time_post_execute,
            )
