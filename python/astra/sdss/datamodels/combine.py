"""Redshift, resample, and combine spectra."""
        ...
        for j, n_res in enumerate(num_pixels_per_resolution_element):
            ...
            if bitmask is not None:
                ...
                output = sincint(
                    ...
                    n_res,
                    [
                        [visit_and_chip(flag_bitmask, i, j), None]
                        for flag_bitmask in separated_bitmasks.values()
                    ],
                )

                for k, (flag, (resampled_bitmask_flag, _)) in enumerate(
                    zip(separated_bitmasks.keys(), output)
                ):
