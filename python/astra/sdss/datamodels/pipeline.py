...
from astra.utils import log, flatten, list_to_dict, expand_path


def create_pipeline_data_product(
    pipeline_name,
    data_product,
    results,
    release: str = "sdss5",
    ...
    **kwargs
    ...
    output_product_filetypes = {
        "mwmVisit": "astraVisit",
        "mwmStar": "astraStar",
    }
    try:
        output_product_filetype_prefix = output_product_filetypes[data_product.filetype]
    except:
        raise ValueError(f"Expected data product of one of: {list(output_product_filetypes.keys())}.")

    primary_hdu = _create_pipeline_data_product_primary_hdu(data_product)

    hdus = [primary_hdu]
    matched_keys, task_ids = ([], [])
    for i, (observatory, instrument) in enumerate(
        ...
        if key not in results or len(results[key]) == 0:
            ...
            hdu_results = list_to_dict(results[key])
            task_ids.extend(hdu_results["task_id"])
            hdus.append(
                create_pipeline_hdu(
                    pipeline_name,
                    data_product,
    ...
    filetype = f"{output_product_filetype_prefix}{pipeline_name}"
    ...
    # The reference task ID will be the earliest task ID in the results
    task_id, source = (min(task_ids), data_product.source)
    ...
    # Create data product
    pipeline_data_product, created = DataProduct.get_or_create(
        release=release, 
        filetype=filetype, 
        kwargs=kwds,
        source=source
    ...
    return pipeline_data_product
    


def _create_pipeline_data_product_primary_hdu(data_product):
    ...
    for k in data_product.kwargs.keys():
        data_model_cards.append((k.upper(), str(data_product.kwargs[k]), None))
    ...
    # Update entries which may not be the same as the input data products.
    primary_hdu_cards[get_index("V_ASTRA")][1] = __version__
    primary_hdu_cards[get_index("CREATED")][1] = datetime.datetime.utcnow().strftime(datetime_fmt)
...
def _old_create_pipeline_product(
    task: Task,
