...
def create_mwm_data_products(
    ...
    if input_data_products is None:
        input_data_products = tuple([dp for dp in source.data_products if dp.filetype in input_filetypes])

    for dp in input_data_products:
        if dp.filetype not in input_filetypes:
            log.warning(f"Ignoring file type '{dp.filetype}' ({dp}: {dp.path}). It's not used for creating MWM Visit/Star products.")
    ...
    apogee_north_visits, apogee_north_star = apogee.create_apogee_hdus(
        [dp for dp in input_data_products \
