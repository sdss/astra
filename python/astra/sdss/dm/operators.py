...
from astra.database.astradb import (database, Task, Bundle, TaskBundle, TaskInputDataProducts, Source, DataProduct, TaskOutputDataProducts)
    ...
    def execute(self):
        ...
        for task, data_products, parameters in self.iterable():
            ...
            hdu_visit_list, hdu_star_list = create_mwm_data_products(
                catalogid,
                input_data_products=data_products
            )
            # Get healpix.
            ...
            try:
                ...
                log.exception(f"Exception when trying to write to either:\n{mwmVisit_path}\n{mwmStar_path}")
                # Delete both.
                for path in (mwmVisit_path, mwmStar_path):
            ...
            log.info(f"Created data products {dp_visit} and {dp_star} for catalogid {catalogid}")
...
class MWMVisitStarFactory(BaseOperator):
    ...
    def execute(self, context):
        ...
        catalogids = map(int, tuple(set(flatten(ti.xcom_pull(task_ids=task.upstream_task_ids)))))
        for catalogid in catalogids:

            log.info(f"Creating mwmVisit and mwmStar products for catalogid={catalogid}")
