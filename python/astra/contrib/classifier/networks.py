...
class OpticalCNN(nn.Module):
    ...
    def __init__(self, in_channels=1, nb_channels=3, nb_classes=None):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Conv1d(in_channels=nb_channels, out_channels=nb_channels, kernel_size=3, stride=2),
            nn.BatchNorm1d(nb_channels),
            nn.ReLU(),
            nn.Dropout(0.1), # 1024 950
            nn.Conv1d(in_channels=nb_channels, out_channels=nb_channels*2, kernel_size=3, stride=2),
            nn.BatchNorm1d(nb_channels*2),
            nn.ReLU(),
            nn.Dropout(0.1), #512 475
            nn.Conv1d(in_channels=nb_channels*2, out_channels=nb_channels*4, kernel_size=3, stride=2),
            nn.BatchNorm1d(nb_channels*4),
            nn.ReLU(),
            nn.Dropout(0.1), #256 237?
            nn.Conv1d(in_channels=nb_channels*4, out_channels=nb_channels*8, kernel_size=3, stride=2),
            nn.BatchNorm1d(nb_channels*8),
            nn.ReLU(),
            nn.Dropout(0.1), #128 118?
            nn.Conv1d(in_channels=nb_channels*8, out_channels=nb_channels*8, kernel_size=3, stride=2, padding=0),
            nn.BatchNorm1d(nb_channels*8),
            nn.ReLU(),
...
class NIRCNN(nn.Module):
    ...
    def __init__(self, nb_channels=3, nb_classes=4):
        ...
        self.model = nn.Sequential(
            ...
            nn.Conv1d(in_channels=nb_channels*8, out_channels=nb_channels*8, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm1d(nb_channels*8),
            nn.ReLU(),
            nn.Dropout(0.1) #64
        )
