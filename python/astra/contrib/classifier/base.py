...
class ClassifySource(TaskInstance):
    ...
    def execute(self, context=None):
        ...
        for task, data_products, parameters in self.iterable():
            ...
            log.info(
                f"Task {task} with data product {data_products} has result {result}"
            )
        ...
        with tqdm(total=total) as pb:
            for (task, *_), result in zip(self.iterable(), results):
                with database.atomic() as tx:
                    ...
                    ClassifySourceOutput.create(task=task, output=output, **result)
                    pb.update()
...
class ClassifyApVisit(TaskInstance):
    ...
    def execute(self):
        ...
        for i, (task, *_) in enumerate(
            ...
            result.update(snr=snrs[i], dithered=dithereds[i])
            output = Output.create()
            TaskOutput.create(output=output, task=task)
            ClassifierOutput.create(task=task, output=output, **result)
            results.append(result)
