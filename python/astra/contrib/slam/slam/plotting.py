...
def plot_mse(s):
    ...
    plt.hist(
        -s.nmse[s.nmse != 0], np.linspace(0, 1, 80), histtype="step", lw=2, label="MSE"
    )
    plt.hist(
        -s.scores[s.nmse != 0],
        np.linspace(0, 1, 80),
    ...
    plt.vlines(
        np.percentile(-s.nmse[s.nmse != 0], [14, 50, 86]),
        *ylim,
        linestyle="--",
        label="14, 50, 86 percentiles"
    )
    plt.xlim(0, 1)
    plt.ylim(*ylim)
    plt.ylabel("Counts")
...
def image(ax, x, y, xbins, ybins, log=True):
    plt.sca(ax)
    c, xe, ye, bn = binned_statistic_2d(x, y, x, statistic="count", bins=[xbins, ybins])
    if log:
        c = np.log10(c)
    plt.imshow(
        c.T,
        origin="lower",
        extent=(*xbins[[0, -1]], *ybins[[0, -1]]),
        cmap=cm.viridis,
        aspect="auto",
    )
    return
...
def compare_labels(
    ...
    if xlims is None:
        xlim1 = np.min(
            np.vstack(
                (np.percentile(X_true, 1, axis=0), np.percentile(X_pred, 1, axis=0))
            ),
            axis=0,
        )
        xlim2 = np.min(
            np.vstack(
                (np.percentile(X_true, 99, axis=0), np.percentile(X_pred, 99, axis=0))
            ),
            axis=0,
        )
        xlims = (xlim2 - xlim1).reshape(-1, 1) * 0.4 * np.array([-1, 1]) + np.vstack(
            (xlim1, xlim2)
        ).T
    if reslims is None:
        reslims = np.repeat(
            np.max(
                np.abs(np.percentile(X_pred - X_true, [1, 99], axis=0).T), axis=1
            ).reshape(-1, 1),
            2,
            axis=1,
        ) * np.array([-1, 1])
        reslims = np.abs(np.diff(reslims, axis=1)) * np.array([-1, 1]) * 0.2 + reslims
    ...
    for i in range(nlabel):
        ...
        plt.hist(data_, bins=bin_edge_, histtype="step", orientation="horizontal")
        ...
        pos_text_y = np.dot(np.array([[0.15, 0.85]]), reslims[i].reshape(-1, 1))
        axs2[i + 1, -1].text(pos_text_x, pos_text_y, "$bias={:.4f}$".format(X_bias[i]))
        pos_text_x = np.dot(np.array([[0.9, 0.1]]), histlim.reshape(-1, 1))
        pos_text_y = np.dot(np.array([[0.30, 0.70]]), reslims[i].reshape(-1, 1))
        axs2[i + 1, -1].text(
            pos_text_x, pos_text_y, "$\\sigma={:.4f}$".format(X_scatter[i])
        )
...
def visual3d(s, wave, diag_dim=()):
    ...
    i_pixel = 5175 - 3900
    # i_pixel = 4861-3900
    x, y, flux = s.single_pixel_diagnostic(i_pixel, s.tr_labels, diag_dim=diag_dim)
    ...
    sdiag_teff = np.arange(4000.0, 8000.0, 100.0)
    sdiag_logg = np.arange(1.0, 5.0, 0.2)
    sdiag_logg = np.arange(-2, 1.0, 0.2)
    msdiag_teff, msdiag_logg = np.meshgrid(sdiag_teff, sdiag_logg)
    msdiag_feh = np.zeros_like(msdiag_teff)
    msdiag_labels = np.array(
        [msdiag_teff.flatten(), msdiag_logg.flatten(), msdiag_feh.flatten()]
    ).T

    H, xedges, yedges = np.histogram2d(
        train_labels[:, 0], train_labels[:, 2], bins=(sdiag_teff, sdiag_logg)
    )
    xcenters = (xedges[:-1] + xedges[1:]) / 2.0
    ycenters = (yedges[:-1] + yedges[1:]) / 2.0
    xmesh, ymesh = np.meshgrid(xcenters, ycenters)

    x, y, flux = s.single_pixel_diagnostic(i_pixel, msdiag_labels, diag_dim=diag_dim)
    ...
    ax.contour(xmesh, ymesh, np.log(H.T), extend3d=False, offset=1.20, color="k")
    # plt.plot(x,y, flux,'b.')
    ax.set_zlim(0.0, 2.0)
    plt.xlabel("Teff")
    plt.ylabel("logg")
