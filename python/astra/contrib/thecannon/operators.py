...
def _select_training_set_data_from_database(
    ...
    if primary_parent == catalogdb.SDSSApogeeAllStarMergeR13:
        ...
        q = session.query(*columns).join(
            ...
            func.trim(catalogdb.SDSSApogeeAllStarMergeR13.apstar_ids) == catalogdb.SDSSDR16ApogeeStar.apstar_id
        )
        ...
        raise NotImplementedError(f"Cannot intelligently figure out what data model keywords will be necessary.")
...
def train_polynomial_model(labels, data, order=2, regularization=0, threads=1):
    ...
    try:
        ...
        log.debug(f"Keeping all items in training set; not checking for missing spectra.")
    else:
        keep = (num_spectra == 1)
        if not all(keep):
            log.warning(f"Excluding {sum(~keep)} objects from the training set that had missing spectra")
