...
class MedianFilterNormalizationWithErrorInflation(MedianNormalizationWithErrorInflation):
    ...
    def __call__(self):
        ...
        n_pixels = np.array([header["NPIX"] for header in read_ferre_headers(expand_path(median_filter_from_task.parameters["header_path"]))][1:])
        _ = n_pixels.cumsum()
        ...
        for output_data_product in median_filter_from_task.output_data_products:
            ...
            kwds = dict(
                ...
                normalised_observed_flux=output["data"]["flux"] / output["data"]["continuum"],
                normalised_observed_flux_err=output["data"]["flux_sigma"] / output["data"]["continuum"],
        ...
        for si, ei in segment_indices:
            ...
            s_index, e_index = spectrum.wavelength.value.searchsorted(output["data"]["wavelength"][si:ei][[0, -1]])
