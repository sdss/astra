...
from astra.contrib.thepayne_che import sobol_seq
...
class Fit:
    def __init__(self, network: Network, Cheb_order, tol=5.0e-4):
    ...
    def run(self, wavelength, norm_spec, spec_err, mask=None, p0=None):
        ...
        if not hasattr(self, "bounds_unscaled"):
            ...
            for i in [0, 1]:
                bounds[i, :nnl] = (self.bounds_unscaled[i, :] - x_min) / (
                    x_max - x_min
                ) - 0.5
        bounds[0, nnl:] = -np.inf
        bounds[1, nnl:] = np.inf
        ...
        for it in range(self.N_presearch_iter):
            ...
            for i in range(self.N_pre_search):
                lbl = np.copy(popt)
                lbl[:nnl] = bounds[0, :nnl] + N_sobol[i, :nnl] * (
                    bounds[1, :nnl] - bounds[0, :nnl]
                )
