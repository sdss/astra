...
class BossSpectrumOperator(BaseOperator):
    ...
    *observed* in the operator execution period.
    ...
    def execute(self, context):
        ...
        N = sum(mask)
        if N == 0:
            raise AirflowSkipException(f"No products for {self} observed between {prev_ds} and {ds}")
...
class MilkyWayMapperOperator(BaseOperator):
    ...
    def __init__(
        self,
        *,
        return_id_type="data_product",
        **kwargs
    ) -> None:
        super().__init__(**kwargs)
        available_id_types = ("data_product", "source")
        if return_id_type not in available_id_types:
            raise ValueError(f"Return identifier type must be one of {available_id_types} (not {return_id_type})")
    ...
    def execute(self, context):
        ...
        q = (
            Target.select(Target.catalogid)
                  ...
                  .join(Carton)
                  .where(
                        Target.catalogid.in_(catalogids)
                    &   Carton.carton.in_(self.cartons)
                  )
        )
