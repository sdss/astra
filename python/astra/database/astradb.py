...
class DataProduct(AstraBaseModel):
    ...
    def path(self):
        ...
        try:
            ...
            _sdss_path_instances[self.release] = SDSSPath(self.release)
            print(f"using new one for {self.release} and {self.filetype}")
            return _sdss_path_instances[self.release].full(self.filetype, **self.kwargs)
        else:
            print(f"using pre-existing one")
...
class Bundle(AstraBaseModel):
    ...
    def instance(self, only_incomplete=False, strict=True, reverse=False):
        from astra.base import TaskInstance
        return TaskInstance.from_bundle(self, only_incomplete=only_incomplete, strict=strict, reverse=reverse)
    ...
    def split(self, N):
        ...
        new_bundles = [Bundle() for i in range(N)]
        with database.atomic():
            Bundle.bulk_create(new_bundles)
        ...
        for i, bundle in enumerate(new_bundles):
            ...
            for task in tasks[si:ei]:
                ids.append((task.id, bundle.id))

        with database.atomic():
            TaskBundle.insert_many([
                { "task_id": task_id, "bundle_id": bundle_id } for task_id, bundle_id in ids
            ]).execute()        
...
class TheCannonOutput(AstraOutputBaseModel):
    ...
    rho_fe_h_na_h = FloatField(null=True)
    ...
    rho_fe_h_mg_h = FloatField(null=True)
    ...
    rho_fe_h_al_h = FloatField(null=True)
    ...
    rho_fe_h_si_h = FloatField(null=True)
    ...
    rho_fe_h_s_h = FloatField(null=True)
    ...
    rho_fe_h_k_h = FloatField(null=True)
    ...
    rho_fe_h_ca_h = FloatField(null=True)
    ...
    rho_fe_h_ti_h = FloatField(null=True)
    ...
    rho_fe_h_v_h = FloatField(null=True)
    ...
    rho_fe_h_cr_h = FloatField(null=True)
    ...
    rho_fe_h_mn_h = FloatField(null=True)
