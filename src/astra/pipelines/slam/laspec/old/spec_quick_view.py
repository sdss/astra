...
def _xtick_pos_lab(n_chunks,
    ...
    for i_chunk in xrange(n_chunks):
        n_xtick_l = int(np.abs((wave_centers[i_chunk] - wave_intervals[i_chunk][0]) / xtick_pos_step[i_chunk]))
        n_xtick_r = int(np.abs((wave_centers[i_chunk] - wave_intervals[i_chunk][1]) / xtick_pos_step[i_chunk]))
