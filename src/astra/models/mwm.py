...
class ApogeeRestFrameVisitSpectrum(MWMVisitMixin, SpectrumMixin):
    ...
    snr = FloatField(null=True, help_text=Glossary.snr)
    in_stack = BooleanField(null=False, help_text=Glossary.in_stack)
    visit_flags = BitField(default=0, help_text=Glossary.visit_flags)
    ...
    flag_bad_pixels = visit_flags.flag(2**0, help_text="Spectrum has many bad pixels (>20%).")
    flag_commissioning = visit_flags.flag(2**1, help_text="Commissioning data (MJD <55761); non-standard configuration; poor LSF.")
    flag_bright_neighbor = visit_flags.flag(2**2, help_text="Star has neighbor more than 10 times brighter.")
    flag_very_bright_neighbor = visit_flags.flag(2**3, help_text="Star has neighbor more than 100 times brighter.")
    flag_low_snr = visit_flags.flag(2**4, help_text="Spectrum has low S/N (<5).")
    # 4-8 inclusive are not defined
    flag_persist_high = visit_flags.flag(2**9, help_text="Spectrum has at least 20% of pixels in high persistence region.")
    flag_persist_med = visit_flags.flag(2**10, help_text="Spectrum has at least 20% of pixels in medium persistence region.")
    flag_persist_low = visit_flags.flag(2**11, help_text="Spectrum has at least 20% of pixels in low persistence region.")
    flag_persist_jump_pos = visit_flags.flag(2**12, help_text="Spectrum has obvious positive jump in blue chip.")
    flag_persist_jump_neg = visit_flags.flag(2**13, help_text="Spectrum has obvious negative jump in blue chip.")
    # 14-15 inclusive are not defined
    flag_suspect_rv_combination = visit_flags.flag(2**16, help_text="RVs from synthetic template differ significantly (~2 km/s) from those from combined template.")
    flag_suspect_broad_lines = visit_flags.flag(2**17, help_text="Cross-correlation peak with template significantly broader than autocorrelation of template.")
    flag_bad_rv_combination = visit_flags.flag(2**18, help_text="RVs from synthetic template differ very significantly (~10 km/s) from those from combined template.")
    flag_rv_reject = visit_flags.flag(2**19, help_text="Rejected visit because cross-correlation RV differs significantly from least squares RV.")
    flag_rv_suspect = visit_flags.flag(2**20, help_text="Suspect visit (but used!) because cross-correlation RV differs slightly from least squares RV.")
    flag_multiple_suspect = visit_flags.flag(2**21, help_text="Suspect multiple components from Gaussian decomposition of cross-correlation.")
    flag_rv_failure = visit_flags.flag(2**22, help_text="RV failure.")
    flag_suspect_rotation = visit_flags.flag(2**23, help_text="Suspect rotation: cross-correlation peak with template significantly broader than autocorretion of template")
    flag_mtpflux_lt_75 = visit_flags.flag(2**24, help_text="Spectrum falls on fiber in MTP block with relative flux < 0.75")
    flag_mtpflux_lt_50 = visit_flags.flag(2**25, help_text="Spectrum falls on fiber in MTP block with relative flux < 0.5")
